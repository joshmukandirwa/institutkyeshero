    ...
    function reprendreBlocage(fin) {
      ...
      timerBlock = setInterval(function () {
        ...
        if (restant <= 0) {
          ...
          cd.textContent = restant;/* ══ PERSISTANCE DU BLOCAGE ══ */
var CLE_TENTATIVES = 'ikg_tentatives';
var CLE_BLOCAGE_FIN = 'ikg_blocage_fin';
var maxTentatives = 3;
var timerBlock;
...
/* ══ CHARGEMENT DEPUIS GOOGLE SHEETS via opensheet (sans CORS) ══ */
var SHEET_ID  = '1734ZsAi_FlzNcoJDSDzNQR5kxHRTa_e--bKHRd_0_HI';
// ⚠️ Remplace "Feuille1" par le vrai nom de ton onglet si différent
var SHEET_URL = 'https://opensheet.elk.sh/' + SHEET_ID + '/Feuille1';
...
function chargerDepuisSheets(callback) {
  ...
  fetch(SHEET_URL)
    .then(function (r) {
      ...
      return r.json(); // ← JSON cette fois, pas CSV
    })
    .then(function (rows) {
      ELEVES = [];
    ...
    .catch(function (err) {
      ...
      // Afficher un message discret dans la console, pas bloquer l'UI
...
function seConnecter() {
  ...
  // Étape 2 — chercher le matricule seul
